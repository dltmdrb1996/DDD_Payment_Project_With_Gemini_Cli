...
- **DDD 최우선 (DDD First)**: 모든 구현은 도메인 주도 설계(DDD) 원칙을 최우선으로 고려합니다. (**#3 DDD 코드 스타일 세부 원칙** 참조)
...
- **테스트 주도 개발 (TDD)**: 핵심 비즈니스 로직(도메인 모델, 도메인 행위) 구현 시 반드시 테스트 코드를 먼저 작성하고 이를 통과하는 코드를 구현합니다. (**#7 테스트 심층 원칙** 참조)
...
# 3 DDD (Domain-Driven Design) 코드 스타일 세부 원칙
